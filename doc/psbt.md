# PSBT Howto for QTY Core

Since QTY Core 0.17, an RPC interface exists for Partially Signed QTY
Transactions (PSBTs, as specified in
[BIP 174](https://github.com/qty/bips/blob/master/bip-0174.mediawiki)).
...
PSBT is an interchange format for QTY transactions that are not fully signed
...
#### Multisig with multiple QTY Core instances
...
QTY Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `qty-cli` is used.
...
Later, when *V* QTY has been received on *Amulti*, and Bob and Carol want to
