...
{
    if (!sig.empty() && sig.size() != QTY_DILITHIUM_SIGNATURE_SIZE + 1) {
 ...
 *
 * See https://qtytalk.org/index.php?topic=8392.msg127623#msg127623
 *
...
bool static IsLowDERSignature(const valtype &vchSig, ScriptError* serror) {
    ...
    // https://qty.stackexchange.com/a/12556:
    ...
    try
    {
        for (; pc < pend; ++opcode_pos) {
            ...
            switch (opcode)
            {
                ...
                {
                    // QTY-AUDIT-048: once DEPLOYMENT_DILITHIUM_P2MR activates, Dilithium opcodes
...
static bool VerifyWitnessProgram(const CScriptWitness& witness, int witversion, const std::vector<unsigned char>& program, unsigned int flags, const BaseSignatureChecker& checker, ScriptError* serror, bool is_p2sh)
{
    ...
    if (witversion == 0) {
        if (program.size() == WITNESS_V0_SCRIPTHASH_SIZE) {
            ...
            // QTY-AUDIT-048: after DEPLOYMENT_DILITHIUM_P2MR activation the witness-v0
