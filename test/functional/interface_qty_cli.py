...
class TestQTYCli(QTYTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Compare responses from getblockchaininfo RPC and `qty-cli getblockchaininfo`")
        ...
        self.log.info("Test -getinfo and qty-cli return all proxies")
        ...
        if self.is_specified_wallet_compiled():
            self.log.info("Test -getinfo and qty-cli getwalletinfo return expected wallet info")
            ...
            assert_equal(Decimal(cli_get_info['Transaction fee rate (-paytxfee) (QTY/kvB)']), wallet_info['paytxfee'])
            assert_equal(Decimal(cli_get_info['Min tx relay fee rate (QTY/kvB)']), network_info['relayfee'])
            ...
            # Test qty-cli -generate with -rpcwallet in multiwallet mode.
