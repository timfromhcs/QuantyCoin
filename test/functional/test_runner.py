...
`test/functional/test_framework/test_framework.py:QTYTestFramework.main`.
...
BASE_SCRIPTS = [
    ...
    'qty_regtest_mining.py',
    'qty_chain_identity.py',
    ...
    'interface_qty_cli.py --legacy-wallet',
    'interface_qty_cli.py --descriptors',
    ...
    'rpc_dumptxoutset.py',
    'tool_scan_legacy_dilithium.py --descriptors',
    ...
    'wallet_bip360_send_paths.py --descriptors',
...
def main():
    ...
    enable_qtyd = config["components"].getboolean("ENABLE_QTYD")
...
def run_tests(*, test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, use_term_control):
    ...
    # Warn if qtyd is already running
    try:
        # pgrep exits with code zero when one or more matching processes found
        if subprocess.run(["pgrep", "-x", "qtyd"], stdout=subprocess.DEVNULL).returncode == 0:
            print("%sWARNING!%s There is already a qtyd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    except OSError:
        # pgrep not supported
        pass
    ...
    # a hard link or a copy on any platform. See https://github.com/qty/qty/pull/27561.
...
def check_script_prefixes():
    ...
    good_prefixes_re = re.compile("^(qty|example|feature|interface|mempool|mining|p2p|rpc|wallet|tool)_")
...
class RPCCoverage():
    ...
    commands per `qty-cli help` (`rpc_interface.txt`).
