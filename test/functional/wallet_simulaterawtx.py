...
class SimulateTxTest(QTYTestFramework):
    ...
    def run_test(self):
        ...
        qty_fee = Decimal(funding["fee"])

        # w0 sees fee + 0.5 qty decrease, w2 sees + 0.5 qty
        assert_approx(w0.simulaterawtransaction([tx1])["balance_change"], -(Decimal("0.5") + qty_fee))
        ...
        #       wallet                  tx3                             tx4
        #       w0                      -0.5 - qty_fee + 0.49999    -0.5 - qty_fee
        #       w1                      0                               +0.49999
        assert_approx(w0.simulaterawtransaction([tx1, tx3])["balance_change"], -Decimal("0.5") - qty_fee + Decimal("0.49999"))
        assert_approx(w1.simulaterawtransaction([tx1, tx3])["balance_change"], 0)
        assert_approx(w0.simulaterawtransaction([tx1, tx4])["balance_change"], -Decimal("0.5") - qty_fee)
        ...
        tx2 = funding["hex"]
        qty_fee2 = Decimal(funding["fee"])
        assert_approx(w0.simulaterawtransaction([tx2])["balance_change"], -(Decimal("1") + qty_fee2))
