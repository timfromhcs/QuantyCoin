# QuantyCoin QTY3 Mainnet Protocol Specification & Consensus Freeze

**Document**: `docs/protocol/QTY3_PROTOCOL_FREEZE.md`  
**Protocol Version**: QTY3 (70030 / Transition compatible with 70020)  
**Status**: **FROZEN & RATIFIED FOR MAINNET**  
**Ratification Date**: 2026-09-05  

---

## 1. Executive Protocol Determination

In accordance with Phase 01 criteria, the transition to **QuantyCoin QTY3** constitutes a safe, backward-compatible, forward-secure consensus evolution:
- **Consensus Compatibility**: Maintains exact continuity with the QTY2 Genesis Block (`00000f7cecd0b1eafaab4d65183f7bd12713b67b6c1c4a30f6bf3f1b8efd30ba`).
- **Post-Quantum Security**: Enforces genuine NIST FIPS 204 ML-DSA-44 lattice cryptography for quantum-resistant transaction authorization with zero insecure fallbacks.
- **Asymmetric Dual-PoW**: First-class support for Lane A (SHA-256D ASIC) and Lane B (RFC 7914 Scrypt General-Purpose).
- **Thermodynamic Chainwork**: Strict fork-choice rule resolving exclusively on cumulative thermodynamic energy.

---

## 2. Immutable Consensus Parameter Freeze Ledger

| Parameter | Frozen Value | Enforcement Scope | Invariant Rule |
| :--- | :--- | :--- | :--- |
| **Protocol Version** | `70020` / `70030` | P2P handshake, block headers | Rejects non-compatible version peers |
| **Network Magic (Mainnet)** | `0x5155414e` (`QUAN`) | P2P wire framing | Rejects frames with alien magic bytes |
...
| **Base Block Subsidy (Lane A)**| `50` QTY ($5 \times 10^9$ sat) | Lane A coinbase reward | 100% subsidy for industrial ASIC lane |
| **Base Block Subsidy (Lane B)**| `25` QTY ($2.5 \times 10^9$ sat)| Lane B coinbase reward | 50% subsidy for consumer CPU/GPU lane |
| **Subsidy Halving Window** | `2,100,000` blocks | Supply emission schedule | Halves subsidy every ~4 years |
| **Total Maximum Supply** | $\le 21,000,000$ QTY | Monetary integrity | Hard supply cap strictly preserved |
...
## 3. Inviolable Security Invariants
...
4. **Non-Confiscatory Migration**: Legacy UTXOs remain spendable during the transition phase, with automated wallet tools facilitating migration to post-quantum custody.
