...
BOOST_AUTO_TEST_CASE(knapsack_solver_test)
{
    ...
    for (int i = 0; i < RUN_TESTS; i++)
    {
        ...
        BOOST_CHECK_EQUAL(result14->GetSelectedValue(), 1 * COIN);  // we should get 1 QTY in 1 coin
        ...
        BOOST_CHECK_EQUAL(result15->GetSelectedValue(), 2 * COIN);  // we should get 2 QTY in 1 coin
 ...
 *  scale factor and QTY's is 16 rather than 4. A standard transaction therefore
...
BOOST_AUTO_TEST_CASE(srd_tests)
{
...

    {
        ...
        const auto& res = SelectCoinsSRD(target, dummy_params, m_node, max_weight, [&](CWallet& wallet) {
            CoinsResult available_coins;
            for (int j = 0; j < 60; ++j) { // 60 UTXO --> 19.8 QTY total, more weight than the budget allows
                add_coin(available_coins, wallet, CAmount(0.33 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            for (int i = 0; i < 10; i++) { // 10 UTXO --> 20 QTY total, cheap enough to build on
                add_coin(available_coins, wallet, CAmount(2 * COIN), CFeeRate(0), 144, false, 0, true);
            }
            return available_coins;
        });
...
BOOST_AUTO_TEST_CASE(check_max_weight)
{
    ...
    {
        // Scenario 1:
        // The actor starts with 1x 50.0 QTY and 1515x 0.033 QTY (~100.0 QTY total) unspent outputs
        // Then tries to spend 49.5 QTY
        // The 50.0 QTY output should be selected, because the transaction would otherwise be too large
        ...
        // Verify that only the 50 QTY UTXO was selected
...

    {
        ...
        // The actor starts with 190x 0.25 QTY and 1000x 0.07 QTY (117.5 QTY total) unspent outputs
        // Then tries to spend 49.5 QTY
        ...
        // its own: the large coins total 47.5 QTY so they fall short, and the
        ...
        // (25 QTY, short) against 2000x 0.025 (1980 inputs, too heavy).
        const auto result = select_coins(
            target, cs_params, cc, [&](CWallet& wallet) {
                CoinsResult available_coins;
                for (int j = 0; j < 190; ++j) {
...

    {
        ...
        // The actor starts with 1515x 0.033 QTY (49.995 QTY total) unspent outputs
...
BOOST_AUTO_TEST_CASE(SelectCoins_effective_value_test)
{
    ...
    CoinsResult available_coins;
    {
        ...
        add_coin(available_coins, *dummyWallet, 100000); // 0.001 QTY
    }

    CAmount target{99900}; // 0.000999 QTY
    ...
    // the 3000 sat/kvB feerate above. At QTY's scale factor the same 148 weight
