...
# TestP2PConn: A peer we use to send messages to qtyd, and store responses.
...
class CompactBlocksTest(QTYTestFramework):
    ...
    # This test actually causes qtyd to (reasonably!) disconnect us, so do this last.
    ...
    # Test that qtyd requests compact blocks when we announce new blocks
    ...
    def test_incorrect_blocktxn_response(self, test_node):
        ...
        # Note that it's possible for qtyd to be smart enough to know we're
    ...
    def test_getblocktxn_handler(self, test_node):
        node = self.nodes[0]
        # qtyd will not send blocktxn responses for blocks whose height is
