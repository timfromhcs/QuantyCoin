...
static void RegisterMetaTypes()
{
    ...
    qRegisterMetaTypeStreamOperators<QTYUnit>("QTYUnit");
#else
    qRegisterMetaType<QTYUnit>("QTYUnit");
#endif
}
...
static void initTranslations(QTranslator &qtTranslatorBase, QTranslator &qtTranslator, QTranslator &translatorBase, QTranslator &translator)
{
    ...
    // Load e.g. qty_de.qm (shortcut "de" needs to be defined in qty.qrc)
    if (translatorBase.load(lang, ":/translations/")) {
        QApplication::installTranslator(&translatorBase);
    }

    // Load e.g. qty_de_DE.qm (shortcut "de_DE" needs to be defined in qty.qrc)
...
static const char* qt_argv = "qty-qt";

QTYApplication::QTYApplication()
...
void QTYApplication::setupPlatformStyle()
{
    // UI per-platform customization
    // This must be done inside the QTYApplication constructor, or after it, because
    // PlatformStyle::instantiate requires a QApplication
    std::string platformName;
    platformName = gArgs.GetArg("-uiplatform", QTYGUI::DEFAULT_UIPLATFORM);
...
#ifdef ENABLE_WALLET
void QTYApplication::createPaymentServer()
{
    paymentServer = new PaymentServer(this);
}
#endif

bool QTYApplication::createOptionsModel(bool resetSettings)
{
...
void QTYApplication::createWindow(const NetworkStyle *networkStyle)
{
    window = new QTYGUI(node(), platformStyle, networkStyle, nullptr);
    connect(window, &QTYGUI::quitRequested, this, &QTYApplication::requestShutdown);
...
void QTYApplication::createSplashScreen(const NetworkStyle *networkStyle)
{
    assert(!m_splash);
...
void QTYApplication::createNode(interfaces::Init& init)
{
    assert(!m_node);
...
bool QTYApplication::baseInitialize()
{
    return node().baseInitialize();
}

void QTYApplication::startThread()
{
    assert(!m_executor);
    m_executor.emplace(node());

    /*  communication to and from thread */
    connect(&m_executor.value(), &InitExecutor::initializeResult, this, &QTYApplication::initializeResult);
    connect(&m_executor.value(), &InitExecutor::shutdownResult, this, [] {
        QCoreApplication::exit(0);
    });
    connect(&m_executor.value(), &InitExecutor::runawayException, this, &QTYApplication::handleRunawayException);
    connect(this, &QTYApplication::requestedInitialize, &m_executor.value(), &InitExecutor::initialize);
    connect(this, &QTYApplication::requestedShutdown, &m_executor.value(), &InitExecutor::shutdown);
}

void QTYApplication::parameterSetup()
{
...
void QTYApplication::InitPruneSetting(int64_t prune_MiB)
{
    optionsModel->SetPruneTargetGB(PruneMiBtoGB(prune_MiB));
}

void QTYApplication::requestInitialize()
{
...
void QTYApplication::requestShutdown()
{
...
void QTYApplication::initializeResult(bool success, interfaces::BlockAndHeaderTipInfo tip_info)
{
    ...
    if (success) {
        ...
        // qty: URIs or payment requests:
        if (paymentServer) {
            connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &QTYGUI::handlePaymentRequest);
            connect(window, &QTYGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
void QTYApplication::handleRunawayException(const QString &message)
{
...
void QTYApplication::handleNonFatalException(const QString& message)
{
...
WId QTYApplication::getMainWinId() const
{
    if (!window)
        return 0;
...
bool QTYApplication::event(QEvent* e)
{
...
static void SetupUIArgs(ArgsManager& argsman)
{
    ...
    argsman.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", QTYGUI::DEFAULT_UIPLATFORM), ArgsManager::ALLOW_ANY | ArgsManager::DEBUG_ONLY, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    gArgs.SetFallbackChainType(ChainType::QTYTEST);
    ...
    /// 6-7. Parse qty.conf, determine network, switch to network specific
    ...
    // qty: links repeatedly have their payment requests routed to this process:
    if (WalletModel::isWalletEnabled()) {
