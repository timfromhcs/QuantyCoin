...
BOOST_AUTO_TEST_CASE(key_io_invalid)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        for (const auto& chain : {ChainType::QTYMAIN, ChainType::QTYTEST, ChainType::QTYSIGNET, ChainType::QTYREGTEST}) {
            SelectParams(chain);
            destination = DecodeDestination(exp_base58string);
...
BOOST_AUTO_TEST_CASE(key_io_testnet_legacy_dilithium_base58_still_valid)
{
    ...
    // Live Testnet-QTY-Mining-Pool coinbase address.
    ...
    SelectParams(ChainType::QTYREGTEST);
    CDilithiumKey key;
