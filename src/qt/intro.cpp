...
bool Intro::showIfNeeded(bool& did_show_intro, int64_t& prune_MiB)
{
    ...
    if(!fs::exists(GUIUtil::QStringToPath(dataDir)) || gArgs.GetBoolArg("-choosedatadir", DEFAULT_CHOOSE_DATADIR) || settings.value("fReset", false).toBool() || gArgs.GetBoolArg("-resetguisettings", false))
    {
        ...
        intro.setWindowIcon(QIcon(":icons/qty"));
        did_show_intro = true;
     ...
     * override -datadir in the qty.conf file in the default data directory
     * (to be consistent with qtyd behavior)
     */
    if(dataDir != GUIUtil::getDefaultDataDirectory()) {
...
void Intro::UpdatePruneLabels(bool prune_checked)
{
    ...
    static constexpr uint64_t nPowTargetSpacing = 1 * 60;  // QTY: 1-minute blocks (from chainparams, which we don't have at this stage)
    ...
    ui->sizeWarningLabel->setText(
        tr("%1 will download and store a copy of the QTY block chain.").arg(PACKAGE_NAME) + " " +
