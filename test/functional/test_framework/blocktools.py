...
# QTY pays 5 QTY per block where Bitcoin pays 50, and on regtest halves every
...
def block_subsidy(height, *, halving_interval=SUBSIDY_HALVING_INTERVAL):
    """Consensus subsidy for the block at `height`, in QTY."""
...
def cumulative_block_subsidy(first_height, last_height, *, halving_interval=SUBSIDY_HALVING_INTERVAL):
    """Total subsidy paid by blocks `first_height` through `last_height`, inclusive, in QTY."""
...
def create_coinbase(height, pubkey=None, *, script_pubkey=None, extra_output_script=None, fees=0, nValue=None):
    ...
    nValue overrides the payout (in QTY) for tests that need a specific or
