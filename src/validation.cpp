...
// GetBlockScriptFlags is declared in validation.h (exported for unit tests).
...
bool MemPoolAccept::PreChecks(ATMPArgs& args, Workspace& ws)
{
    ...
    if (!ancestors) {
        ...
        // this, see https://lists.linuxfoundation.org/pipermail/qty-dev/2018-November/016518.html
...
CAmount GetBlockSubsidy(int nHeight, const Consensus::Params& consensusParams)
{
    ...
    CAmount nSubsidy = 5 * COIN; // QTY: 5 QTY/block (1/10th Bitcoin, compensated by 10x more blocks)
    // Subsidy is cut in half every 2,100,000 blocks (~4 years at 1-min blocks).
    nSubsidy >>= halvings;
    return nSubsidy;
}
                       ...
                       std::vector<CScriptCheck>* pvChecks, bool policy_flags)
{
    ...
    for (unsigned int i = 0; i < tx.vin.size(); i++) {
        ...
        if (pvChecks) {
            ...
            //
            // Dropping it also removes a trap for QTY's testnet policy-ahead
...
DisconnectResult Chainstate::DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // See https://github.com/qty/qty/issues/22596 for additional information.
...
unsigned int GetBlockScriptFlags(const CBlockIndex& block_index, const ChainstateManager& chainman)
{
    ...
    // QTY: Modified to conditionally include WITNESS and TAPROOT based on deployment status
    uint32_t flags{SCRIPT_VERIFY_P2SH};

    // QTY: Only enable Witness (SegWit) if the deployment is active
    if (DeploymentActiveAt(block_index, chainman, Consensus::DEPLOYMENT_SEGWIT)) {
        flags |= SCRIPT_VERIFY_WITNESS;
    }

    // QTY: Only enable Taproot if the deployment is active
    ...
    // QTY: Dilithium, P2MR, and P2MR-only must never be clearable via
    ...
    // QTY-AUDIT-048: restrict Dilithium opcodes to P2MR tapscript after activation
    ...
    const std::string& warning_messages) EXCLUSIVE_LOCKS_REQUIRED(::cs_main)
{
    ...
    // reaches it -- a block has to connect first. QTY's 60s spacing makes this
...
bool ChainstateManager::ProcessNewBlock(const std::shared_ptr<const CBlock>& block, bool force_processing, bool min_pow_checked, bool* new_block)
{
...

    {
        ...
        // https://lists.linuxfoundation.org/pipermail/qty-dev/2019-February/016697.html.  Because CheckBlock() is
