...
const char * const QTY_CONF_FILENAME = "qty.conf";
const char * const QTY_SETTINGS_FILENAME = "settings.json";
...
std::set<std::string> ArgsManager::GetUnsuitableSectionOnlyArgs() const
{
    ...
    if (m_network == ChainTypeToString(ChainType::QTYMAIN)) return std::set<std::string> {};
...
std::list<SectionInfo> ArgsManager::GetUnrecognizedSections() const
{
    ...
    static const std::set<std::string> available_sections{
        ChainTypeToString(ChainType::QTYREGTEST),
        ChainTypeToString(ChainType::QTYSIGNET),
        ChainTypeToString(ChainType::QTYTEST),
        ChainTypeToString(ChainType::QTYMAIN),
    };
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //qty-tx using stdin
...
{
    fs::path settings = GetPathArg("-settings", QTY_SETTINGS_FILENAME);
    if (settings.empty()) {
        return false;
    }
    if (backup) {
...
{
    // Windows: C:\Users\Username\AppData\Roaming\QTY
    // macOS: ~/Library/Application Support/QTY
    // Unix-like: ~/.qty
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "QTY";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/QTY";
#else
    // Unix-like
    return pathRet / ".qty";
#endif
...
std::variant<ChainType, std::string> ArgsManager::GetChainArg() const
{
    ...
    if (fBtqRegTest) return ChainType::QTYREGTEST;
    if (fBtqSigNet) return ChainType::QTYSIGNET;
    if (fBtqTest) return ChainType::QTYTEST;
    LOCK(cs_args);
...
{
    return m_network == ChainTypeToString(ChainType::QTYMAIN) || m_network_only_args.count(arg) == 0;
}
