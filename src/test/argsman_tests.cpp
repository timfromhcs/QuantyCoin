...
BOOST_AUTO_TEST_CASE(util_ReadConfigStream)
{
    ...
    test_args.SelectConfigNetwork(ChainTypeToString(ChainType::QTYMAIN));
    BOOST_CHECK(test_args.GetArg("-d", "xxx") == "e");
    BOOST_CHECK(test_args.GetArgs("-ccc").size() == 2);
    BOOST_CHECK(test_args.GetArg("-h", "xxx") == "0");
            ...
            ForEachNoDup(conf_actions, SET, SECTION_NEGATE, [&] {
                for (bool soft_set : {false, true}) {
                    for (bool force_set : {false, true}) {
                        for (const std::string& section : {ChainTypeToString(ChainType::QTYMAIN), ChainTypeToString(ChainType::QTYTEST), ChainTypeToString(ChainType::QTYSIGNET)}) {
                            for (const std::string& network : {ChainTypeToString(ChainType::QTYMAIN), ChainTypeToString(ChainType::QTYTEST), ChainTypeToString(ChainType::QTYSIGNET)}) {
...
BOOST_FIXTURE_TEST_CASE(util_ArgsMerge, ArgsMergeTestingSetup)
{
    ...
    //
    //   ARGS_MERGE_TEST_OUT=results.txt ./test_qty --run_test=util_tests/util_ArgsMerge
    //
...
BOOST_FIXTURE_TEST_CASE(util_ChainMerge, ChainMergeTestingSetup)
{
    ...
    // If check below fails, should manually dump the results with:
    //
    //   CHAIN_MERGE_TEST_OUT=results.txt ./test_qty --run_test=util_tests/util_ChainMerge
    //
    // And verify diff against previous results to make sure the changes are expected.
    //
    // Results file is formatted like:
