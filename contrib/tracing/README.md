...
Tracing (USDT) support for QTY Core on Linux using. For more information on
USDT support in QTY Core see the [USDT documentation].
...
The bpftrace examples contain a relative path to the `qtyd` binary. By
default, the scripts should be run from the repository-root and assume a
self-compiled `qtyd` binary. The paths in the examples can be changed, for
example, to point to release builds if needed. See the
[QTY Core USDT documentation] on how to list available tracepoints in your
`qtyd` binary.

[QTY Core USDT documentation]: ../../doc/tracing.md#listing-available-tracepoints
...
```
$ python3 contrib/tracing/p2p_monitor.py ./src/qtyd
```
...
QTY P2P messages can be larger than 32kb (e.g. `tx`, `block`, ...). The
...
```
$ python3 contrib/tracing/log_raw_p2p_msgs.py ./src/qtyd
```
...
In a different terminal, starting QTY Core in SigNet mode and with
re-indexing enabled.

```
$ ./src/qtyd -signet -reindex
```
...
```bash
$ python3 contrib/tracing/log_utxocache_flush.py ./src/qtyd
```
...
```console
$ python3 contrib/tracing/mempool_monitor.py ./src/qtyd
```
