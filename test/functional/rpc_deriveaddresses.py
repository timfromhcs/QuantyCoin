...
class DeriveaddressesTest(QTYTestFramework):
    ...
    def run_test(self):
        ...
        address = "qcqty1qjqmxmkpmxt80xz4y3746zgt0q3u3ferrl6ksee"
        ...
        assert_equal(self.nodes[0].deriveaddresses(ranged_descriptor, [1, 2]), ["qcqty1qhku5rq7jz8ulufe2y6fkcpnlvpsta7rqm67zef", "qcqty1qpgptk2gvshyl0s9lqshsmx932l9ccsv25mqyfd"])
        assert_equal(self.nodes[0].deriveaddresses(ranged_descriptor, 2), [address, "qcqty1qhku5rq7jz8ulufe2y6fkcpnlvpsta7rqm67zef", "qcqty1qpgptk2gvshyl0s9lqshsmx932l9ccsv25mqyfd"])
        ...
        # Before #26275, qtyd would crash when deriveaddresses was
        # called with derivation index 2147483647, which is the maximum
        # positive value of a signed int32, and - currently - the
        # maximum value that the deriveaddresses qty RPC call
        # accepts as derivation index.
        assert_equal(self.nodes[0].deriveaddresses(descsum_create("wpkh(tprv8ZgxMBicQKsPd7Uf69XL1XwhmjHopUGep8GuEiJDZmbQz6o58LninorQAfcKZWARbtRtfnLcJ5MQ2AtHcQJCCRUcMRvmDUjyEmNUWwx8UbK/1/1/*)"), [2147483647, 2147483647]), ["qcqty1qtzs23vgzpreks5gtygwxf8tv5rldxvvs2x29z9"])
