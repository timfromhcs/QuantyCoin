...
"""Script for verifying QTY Core release binaries.

This script attempts to download the sum file SHA256SUMS and corresponding
signature file SHA256SUMS.asc from qtycore.org and qty.org and
compares them.
...
The builder keys are available in the guix.sigs repo:
    ...
    https://github.com/qty-core/guix.sigs/tree/main/builder-keys
...
HOST1 = "https://qtycore.org"
HOST2 = "https://qty.org"
VERSIONPREFIX = "qty-core-"
...
def verify_shasums_signature(
   ...
   ReturnCode, t.List[SigData], t.List[SigData], t.List[SigData], t.List[SigData]
    ...
    # which pubkeys within the QTY community do we trust for the purposes of
    # binary verification?
    trusted_keys = set()
...
def verify_published_handler(args: argparse.Namespace) -> ReturnCode:
    WORKINGDIR = Path(tempfile.gettempdir()) / f"qty_verify_binaries.{args.version}"
    ...
    # remove binaries that are known not to be hosted by qtycore.org
...
def main():
    ...
    pub_parser.add_argument(
        'version', type=str, help=(
            f'version of the qty release to download; of the format '
    ...
    pub_parser.add_argument(
        ...
        help=(
            ...
            '(Sometimes qty.org lags behind qtycore.org.)')
    )
