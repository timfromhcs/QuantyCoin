...
_qty_cli() {
    ...
    # save and use original argument to invoke qty-cli for -help, help and RPC
    # as qty-cli might not be in $PATH
    qty_cli="$1"
    ...
    case "$cur" in
        ...
        *)
            ...
            if [[ -z "$cur" || "$cur" =~ ^- ]]; then
                helpopts=$($qty_cli -help 2>&1 | awk '$1 ~ /^-/ { sub(/=.*/, "="); print $1 }' )
            fi

            # only parse help if senseful
            if [[ -z "$cur" || "$cur" =~ ^[a-z] ]]; then
                commands=$(_qty_rpc help 2>/dev/null | awk '$1 ~ /^[a-z]/ { print $1; }')
            fi
