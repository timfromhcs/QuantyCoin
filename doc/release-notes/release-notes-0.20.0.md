...
QTY Core version 0.20.0 is now available from:

  <https://qtycore.org/bin/qty-core-0.20.0/>
...
#### Removal of BIP61 reject network messages from QTY Core

The `-enablebip61` command line option to enable BIP61 has been removed.
(#17004)

This feature has been disabled by default since QTY Core version 0.18.0.
...
- Testing or debugging of implementations of the QTY P2P network protocol
  should be done by inspecting the log messages that are produced by a recent
  version of QTY Core. QTY Core logs debug messages
...
- OpenSSL is no longer used by QTY Core.  (#17265)

- BIP70 support has been fully removed from QTY Core. The
...
- A new `-asmap` configuration option has been added to diversify the
  ...
  single ASN.  See [issue #16599](https://github.com/qty/qty/issues/16599),
  [PR #16702](https://github.com/qty/qty/pull/16702), and the
  `qtyd help` for more information.  This option is experimental and
...
- All custom settings configured when QTY Core starts are now
...
- The "Start QTY Core on system login" option has been removed on macOS.
  (#17567)
...
- A "known bug" [announced](https://qtycore.org/en/releases/0.18.0/#wallet-gui)
  in the release notes of version 0.18 has been fixed.  The issue
  affected anyone who simultaneously used multiple QTY Core wallets
...
- For watch-only wallets, creating a new transaction in the Send screen
  ...
  automatically copy a Partially-Signed QTY Transaction (PSBT) to
  the system clipboard.  This can then be pasted into an external
  program such as [HWI](https://github.com/qty-core/HWI) for
  signing.  Future versions of QTY Core should support a GUI option
...
- QTY Core's automatically-generated source code documentation is
  now available at https://doxygen.qtycore.org.  (#17596)
...
- The `qty-cli` utility used with the `-getinfo` parameter now
...
- The `qty-cli` utility now accepts a `-stdinwalletpassphrase`
...
- #17068 Always generate `qtystrings.cpp` on `make translate` (D4nte)
...
- #17741 Included `test_qty-qt` in msvc build (sipsorcery)
...
- #17910 Remove double `LIBQTY_SERVER` linking (fanquake)
...
- #18397 Fix libevent linking for `bench_qty` binary (hebasto)
- #18426 scripts: `Previous_release`: improve behaviour on failed download (theStack)
- #18429 Remove double `LIBQTY_SERVER` from bench-Makefile (brakmic)
...
- #18702 Fix ASLR for qty-cli on Windows (fanquake)
...
- #17206 Add testcase to simulate qty schema in leveldb (adamjonas)
...
- #17330 Add `shrinkdebugfile=0` to regtest qty.conf (sdaftuar)
...
- #18022 Fix appveyor `test_qty` build of `*.raw` (MarcoFalke)
...
- #18166 ci: Run fuzz testing test cases (qty-core/qa-assets) under valgrind to catch memory errors (practicalswift)
...
- #18430 ci: Only clone qty-core/qa-assets when fuzzing (MarcoFalke)
...
- #16983 Add detailed info about QTY Core files (hebasto)
...
- #17119 Fix broken qty-cli examples (andrewtoth)
...
- #17503 Remove bitness from qty-qt help message and manpage (laanwj)
- #17539 Update and improve Developer Notes (hebasto)
- #17561 Changed MiniUPnPc link to https in dependencies.md (sandakersmann)
- #17596 Change doxygen URL to doxygen.qtycore.org (laanwj)
...
- #17648 Rename wallet-tool references to qty-wallet (hel-o)
...
- #18341 Replace remaining literal QTY with `CURRENCY_UNIT` (domob1812)
...
- #16115 On qtyd startup, write config args to debug.log (LarryRuane)
...
- #17650 util: Remove unwanted fields from qty-cli -getinfo (malevolent)
