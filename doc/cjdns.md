# CJDNS support in QTY Core

It is possible to run QTY Core over CJDNS, an encrypted IPv6 network that
...
done outside of QTY Core, similarly to a VPN (either in the host/OS or on
...
redundancy and robustness for both the QTY network and individual nodes.
...
before it will work with your QTY Core node. This is described in steps
...
## Run QTY Core with CJDNS

Once you are connected to the CJDNS network, the following QTY Core
...
When enabled, this option tells QTY Core that it is running in an
...
IPv6 local network. This helps QTY Core perform better address management:
...
CJDNS support was added to QTY Core in version 23.0 and there may be fewer
CJDNS peers than Tor or IP ones. You can use `qty-cli -addrinfo` to see the
...
[doc/tor.md](https://github.com/qty/qty/blob/master/doc/tor.md) for
details.

## CJDNS-related information in QTY Core

There are several ways to see your CJDNS address in QTY Core:
- in the "Local addresses" output of CLI `-netinfo`
- in the "localaddresses" output of RPC `getnetworkinfo`

To see which CJDNS peers your node is connected to, use `qty-cli -netinfo 4`
or the `getpeerinfo` RPC (i.e. `qty-cli getpeerinfo`).

You can use the `getnodeaddresses` RPC to fetch a number of CJDNS peers known to your node; run `qty-cli help getnodeaddresses` for details.
