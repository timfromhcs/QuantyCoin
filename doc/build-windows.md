...
Below are some notes on how to build QTY Core for Windows.

The options known to work for building QTY Core on Windows are:
...
Note that for WSL the QTY Core source path MUST be somewhere in the default mount file system, for
example /usr/src/qty, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\qty`, for example:

    make install DESTDIR=/mnt/c/workspace/qty
