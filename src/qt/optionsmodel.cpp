...
/** Call node.updateRwSetting() with QTY 22.x workaround. */
static void UpdateRwSetting(interfaces::Node& node, OptionsModel::OptionID option, const std::string& suffix, const common::SettingsValue& value)
{
    if (value.isNum() &&
        ...
        // because QTY 22.x releases try to read these specific settings as
        ...
        // in later releases by https://github.com/qty/qty/pull/24498.
        // If new numeric settings are added, they can be written as numbers
        // instead of strings, because qty 22.x will not try to read these.
...
//! Convert enabled/size values to qty -prune setting.
...
//! Get pruning enabled value to show in GUI from qty -prune setting.
...
//! Get pruning size value to show in GUI from qty -prune setting. If
...
bool OptionsModel::Init(bilingual_str& error)
{
    ...
    // Display
    if (!settings.contains("DisplayQTYUnit")) {
        settings.setValue("DisplayQTYUnit", QVariant::fromValue(QTYUnit::QTY));
    }
    QVariant unit = settings.value("DisplayQTYUnit");
    if (unit.canConvert<QTYUnit>()) {
        m_display_qty_unit = unit.value<QTYUnit>();
    } else {
        m_display_qty_unit = QTYUnit::QTY;
        settings.setValue("DisplayQTYUnit", QVariant::fromValue(m_display_qty_unit));
    }
...
void OptionsModel::SetPruneTargetGB(int prune_target_gb)
{
    ...
    // from saved value. Avoid writing settings.json if qty.conf value
...
#endif
    case DisplayUnit:
        return QVariant::fromValue(m_display_qty_unit);
...
{
    if (new_unit.isNull() || new_unit.value<QTYUnit>() == m_display_qty_unit) return;
    m_display_qty_unit = new_unit.value<QTYUnit>();
    QSettings settings;
    settings.setValue("DisplayQTYUnit", QVariant::fromValue(m_display_qty_unit));
    Q_EMIT displayUnitChanged(m_display_qty_unit);
}
...
void OptionsModel::checkAndMigrate()
{
    ...
    if (settingsVersion < CLIENT_VERSION)
    {
        ...
        // see https://github.com/qty/qty/pull/8273
    ...
    // (https://github.com/qty-core/gui/issues/567).
