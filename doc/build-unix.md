UNIX BUILD NOTES
====================
Some notes on how to build QTY Core in Unix.
...
memory available when compiling QTY Core. On systems with less, gcc can be
...
To build QTY Core without wallet, see [*Disable-wallet mode*](#disable-wallet-mode)
...
If you want to build qty-qt, make sure that the required packages for Qt development
...
Once these are installed, they will be found by configure and a qty-qt executable will be
built by default.
...
To build with Qt 5 you need the following:
    ...
    sudo dnf install qt5-qttools-devel qt5-qtyase-devel
...
...
to: /path/to/qty/depends/x86_64-pc-linux-gnu
```
and configure using the following:
```bash
export BDB_PREFIX="/path/to/qty/depends/x86_64-pc-linux-gnu"
...
Disable-wallet mode
--------------------
When the intention is to only run a P2P node, without a wallet, QTY Core can
