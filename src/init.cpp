 ...
 */
static const char* QTY_PID_FILENAME = "qtyd.pid";

static fs::path GetPidFile(const ArgsManager& args)
{
    return AbsPathForConfigVal(args, args.GetPathArg("-pid", QTY_PID_FILENAME));
}
...
void SetupServerArgs(ArgsManager& argsman)
{
    ...
    const auto defaultBaseParams = CreateBaseChainParams(ChainType::QTYMAIN);
    const auto testnetBaseParams = CreateBaseChainParams(ChainType::QTYTEST);
    const auto signetBaseParams = CreateBaseChainParams(ChainType::QTYSIGNET);
    const auto regtestBaseParams = CreateBaseChainParams(ChainType::QTYREGTEST);
    const auto defaultChainParams = CreateChainParams(argsman, ChainType::QTYMAIN);
    const auto testnetChainParams = CreateChainParams(argsman, ChainType::QTYTEST);
    const auto signetChainParams = CreateChainParams(argsman, ChainType::QTYSIGNET);
    const auto regtestChainParams = CreateChainParams(argsman, ChainType::QTYREGTEST);
    ...
    argsman.AddArg("-conf=<file>", strprintf("Specify path to read-only configuration file. Relative paths will be prefixed by datadir location (only useable from command line, not configuration file) (default: %s)", QTY_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-allowignoredconf", strprintf("For backwards compatibility, treat an unused %s file in the datadir as a warning, not an error.", QTY_CONF_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-pid=<file>", strprintf("Specify pid file. Relative paths will be prefixed by a net-specific datadir location. (default: %s)", QTY_PID_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
    ...
    argsman.AddArg("-settings=<file>", strprintf("Specify path to dynamic settings data file. Can be disabled with -nosettings. File is written at runtime and not meant to be edited by users (use %s instead for custom settings). Relative paths will be prefixed by datadir location. (default: %s)", QTY_CONF_FILENAME, QTY_SETTINGS_FILENAME), ArgsManager::ALLOW_ANY, OptionsCategory::OPTIONS);
#if HAVE_SYSTEM
    ...
    // https://github.com/qty/qty/pull/23542 have become widespread.
...
int nFD;
ServiceFlags nLocalServices = ServiceFlags(NODE_NETWORK_LIMITED | NODE_WITNESS | NODE_QTY_DILITHIUM);
...
{
    // Make sure only a single QTY process is using the data directory.
...
bool AppInitMain(NodeContext& node, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if (args.IsArgSet("-datadir") && !args.GetPathArg("-datadir").is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the "
                  "current working directory '%s'. This is fragile, because if qty is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if qty is started while in a temporary directory.\n",
    ...
    if (!peerman_opts.ignore_incoming_txs) {
        ...
        if (read_stale_estimates && (chainparams.GetChainType() != ChainType::QTYREGTEST)) {
