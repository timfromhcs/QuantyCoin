#!/usr/bin/env python3
# Copyright (c) 2014-2022 The QTY Core developers
...
class QTYTestMetaClass(type):
    """Metaclass for QTYTestFramework.

    Ensures that any attempt to register a subclass of `QTYTestFramework`
    adheres to a standard whereby the subclass overrides `set_test_params` and
    `run_test` but DOES NOT override either `__init__` or `main`. If any of
    those standards are violated, a ``TypeError`` is raised."""

    def __new__(cls, clsname, bases, dct):
        if not clsname == 'QTYTestFramework':
            if not ('run_test' in dct and 'set_test_params' in dct):
                raise TypeError("QTYTestFramework subclasses must override "
                                "'run_test' and 'set_test_params'")
            if '__init__' in dct or 'main' in dct:
                raise TypeError("QTYTestFramework subclasses may not override "
                                "'__init__' or 'main'")

        return super().__new__(cls, clsname, bases, dct)


class QTYTestFramework(metaclass=QTYTestMetaClass):
    """Base class for a qty test script.

    Individual qty test scripts should subclass this class and override the set_test_params() and run_test() methods.
    ...
    def parse_args(self):
        ...
        parser.add_argument("--nocleanup", dest="nocleanup", default=False, action="store_true",
                            help="Leave qtyds and test.* datadir on exit or error")
        parser.add_argument("--noshutdown", dest="noshutdown", default=False, action="store_true",
                            help="Don't stop qtyds after the test execution")
        ...
        parser.add_argument("--usecli", dest="usecli", default=False, action="store_true",
                            help="use qty-cli instead of RPC for all commands")
    ...
    def set_binary_paths(self):
        ...
        binaries = {
            "qtyd": ("qtyd", "QTYD"),
            "qty-cli": ("qtycli", "QTYCLI"),
            "qty-util": ("qtyutil", "QTYUTIL"),
            "qty-wallet": ("qtywallet", "QTYWALLET"),
        }
    ...
    def shutdown(self):
        ...
        if not self.options.noshutdown:
            ...
            self.log.info("Note: qtyds were not stopped and may still be running")
        ...
        rpc_logger = logging.getLogger("QTYRPC")
    ...
    def add_nodes(self, num_nodes: int, extra_args=None, *, rpchost=None, binary=None, binary_cli=None, versions=None):
        ...
        if binary is None:
            binary = [get_bin_from_version(v, 'qtyd', self.options.qtyd) for v in versions]
        if binary_cli is None:
            binary_cli = [get_bin_from_version(v, 'qty-cli', self.options.qtycli) for v in versions]
    ...
    def stop_node(self, i, expected_stderr='', wait=0):
        """Stop a qtyd test node"""
        self.nodes[i].stop_node(expected_stderr, wait=wait)

    def stop_nodes(self, wait=0):
        """Stop multiple qtyd test nodes"""
        for node in self.nodes:
    ...
    def _start_logging(self):
        ...
        ch.setLevel(ll)
        # Format logs the same as qtyd's debug.log with microprecision (so log files can be concatenated and sorted)
    ...
    def _initialize_chain(self):
        ...
        for i in range(self.num_nodes):
            ...
            initialize_datadir(self.options.tmpdir, i, self.chain, self.disable_autoconnect)  # Overwrite port/rpcport in qty.conf
    ...
    def skip_if_no_qtyd_tracepoints(self):
        """Skip the running test if qtyd has not been compiled with USDT tracepoint support."""
        if not self.is_usdt_compiled():
            raise SkipTest("qtyd has not been built with USDT tracepoints enabled.")
    ...
    def skip_if_no_qtyd_zmq(self):
        """Skip the running test if qtyd has not been compiled with zmq support."""
        if not self.is_zmq_compiled():
            raise SkipTest("qtyd has not been built with zmq enabled.")
    ...
    def skip_if_no_wallet_tool(self):
        """Skip the running test if qty-wallet has not been compiled."""
        if not self.is_wallet_tool_compiled():
            raise SkipTest("qty-wallet has not been compiled")

    def skip_if_no_qty_util(self):
        """Skip the running test if qty-util has not been compiled."""
        if not self.is_qty_util_compiled():
            raise SkipTest("qty-util has not been compiled")

    def skip_if_no_cli(self):
        """Skip the running test if qty-cli has not been compiled."""
        if not self.is_cli_compiled():
            raise SkipTest("qty-cli has not been compiled.")
    ...
    def is_cli_compiled(self):
        """Checks whether qty-cli was compiled."""
    ...
    def is_wallet_tool_compiled(self):
        """Checks whether qty-wallet was compiled."""
        return self.config["components"].getboolean("ENABLE_WALLET_TOOL")

    def is_qty_util_compiled(self):
        """Checks whether qty-util was compiled."""
        return self.config["components"].getboolean("ENABLE_QTY_UTIL")
