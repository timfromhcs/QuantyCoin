# User-space, Statically Defined Tracing (USDT) for QTY Core

QTY Core includes statically defined tracepoints to allow for more
...
**Note**: QTY Core uses temporary clones of the _main_ UTXO cache
...
## Adding tracepoints to QTY Core
...
Multiple tools can list the available tracepoints in a `qtyd` binary with
USDT support.

### GDB - GNU Project Debugger

To list probes in QTY Core, use `info probes` in `gdb`:
...
stap net        inbound_message  0x000000000014419e /src/qtyd
stap net        outbound_message 0x0000000000107c05 /src/qtyd
stap validation block_connected  0x00000000002fb10c /src/qtyd
…
```

### With `readelf`

The `readelf` tool can be used to display the USDT tracepoints in QTY Core.
Look for the notes with the description `NT_STAPSDT`.

```
$ readelf -n ./src/qtyd | grep NT_STAPSDT -A 4 -B 2
