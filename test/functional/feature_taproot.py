...
def spenders_taproot_active():
    ...
    # == Test case for https://github.com/qty/qty/issues/24765 ==
...
class TaprootTest(QTYTestFramework):
    ...
    def run_test(self):
        # QTY: Taproot/SegWit are disabled; skip this test
        info = self.nodes[0].getblockchaininfo()
        softforks = info.get('softforks', {})
        if not softforks or not softforks.get('taproot', {}).get('active', False):
            raise SkipTest("Taproot is disabled on QTY; skipping feature_taproot")
