...
#ifndef QTY_PROTOCOL_H
#define QTY_PROTOCOL_H
...
/**
 * QTY protocol message types. When adding new message types, don't forget
...
enum ServiceFlags : uint64_t {
    ...
    // set by all QTY Core non pruned nodes, and is unset by SPV clients or other light clients.
    NODE_NETWORK = (1 << 0),
    ...
    // qty-development mailing list. Remember that service bits are just
    // unauthenticated advertisements, so your code must be robust against
    // collisions and other cases where nodes may be advertising a service they
    // do not actually support. Other service bits should be allocated via the
    // BIP process.

    // NODE_QTY_DILITHIUM indicates a QTY node that validates Dilithium
