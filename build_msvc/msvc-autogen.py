...
def parse_config_into_btc_config():
    ...
    config_dict["PACKAGE_STRING"] = f"\"QTY Core {version}\""

    with open(os.path.join(SOURCE_DIR,'../build_msvc/qty_config.h.in'), "r", encoding="utf8") as template_file:
    ...
    with open(os.path.join(SOURCE_DIR,'../build_msvc/qty_config.h'), "w", encoding="utf8") as btc_config:
...
def main():
    parser = argparse.ArgumentParser(description='QTY-core msbuild configuration initialiser.')
    ...
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/qty_config.h'), os.path.join(SOURCE_DIR, 'config/qty-config.h'))
