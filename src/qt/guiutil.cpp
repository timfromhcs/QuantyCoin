...
#include <qt/qtyaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a QTY address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new QTYAddressEntryValidator(parent));
    widget->setCheckValidator(new QTYAddressCheckValidator(parent));
}
...
bool parseQTYURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no qty: URI
    if(!uri.isValid() || uri.scheme() != QString("qty"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if (!QTYUnits::parse(QTYUnit::QTY, i->second, &rv.amount)) {
                    return false;
                }
            }
            fShouldReturnFalse = false;
...
bool parseQTYURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parseQTYURI(uriInstance, out);
}

QString formatQTYURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("qty:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(QTYUnits::format(QTYUnit::QTY, info.amount, false, QTYUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openQTYConf()
{
    ...
    /* Open qty.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == ChainType::QTYMAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "QTY.lnk";
    if (chain == ChainType::QTYTEST) // Remove this special case when testnet CBaseChainParams::DataDir() is incremented to "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "QTY (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / fs::u8path(strprintf("QTY (%s).lnk", ChainTypeToString(chain)));
}
...
fs::path static GetAutostartFilePath()
{
    ChainType chain = gArgs.GetChainType();
    if (chain == ChainType::QTYMAIN)
        return GetAutostartDir() / "qty.desktop";
    return GetAutostartDir() / fs::u8path(strprintf("qty-%s.desktop", ChainTypeToString(chain)));
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        ChainType chain = gArgs.GetChainType();
        // Write a qty.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == ChainType::QTYMAIN)
            optionFile << "Name=QTY\n";
        else
            optionFile << strprintf("Name=QTY (%s)\n", ChainTypeToString(chain));
