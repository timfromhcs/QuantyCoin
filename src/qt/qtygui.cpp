// Copyright (c) 2011-2022 The QTY Core developers
...
const std::string QTYGUI::DEFAULT_UIPLATFORM =
...
QTYGUI::QTYGUI(interfaces::Node& node, const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    m_node(node),
    ...
    m_network_style(networkStyle)
{
    ...
    if(enableWallet)
    {
        ...
        connect(walletFrame, &WalletFrame::createWalletButtonClicked, this, &QTYGUI::createWallet);
    ...
    connect(labelBlocksIcon, &GUIUtil::ClickableLabel::clicked, this, &QTYGUI::showModalOverlay);
    connect(progressBar, &GUIUtil::ClickableProgressBar::clicked, this, &QTYGUI::showModalOverlay);
...
void QTYGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a QTY address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and qty: URIs)"));
    ...
    connect(overviewAction, &QAction::triggered, this, &QTYGUI::gotoOverviewPage);
    connect(sendCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(sendCoinsAction, &QAction::triggered, [this]{ gotoSendCoinsPage(); });
    connect(receiveCoinsAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(receiveCoinsAction, &QAction::triggered, this, &QTYGUI::gotoReceiveCoinsPage);
    connect(historyAction, &QAction::triggered, [this]{ showNormalIfMinimized(); });
    connect(historyAction, &QAction::triggered, this, &QTYGUI::gotoHistoryPage);
#endif // ENABLE_WALLET
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your QTY addresses to prove you own them"));
    verifyMessageAction = new QAction(tr("&Verify message…"), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified QTY addresses"));
    m_load_psbt_action = new QAction(tr("&Load PSBT from file…"), this);
    m_load_psbt_action->setStatusTip(tr("Load Partially Signed QTY Transaction"));
    m_load_psbt_clipboard_action = new QAction(tr("Load PSBT from &clipboard…"), this);
    m_load_psbt_clipboard_action->setStatusTip(tr("Load Partially Signed QTY Transaction from clipboard"));
    ...
    openAction->setStatusTip(tr("Open a qty: URI"));
    ...
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible QTY command-line options").arg(PACKAGE_NAME));
    ...
    connect(quitAction, &QAction::triggered, this, &QTYGUI::quitRequested);
    connect(aboutAction, &QAction::triggered, this, &QTYGUI::aboutClicked);
    connect(aboutQtAction, &QAction::triggered, qApp, QApplication::aboutQt);
    connect(optionsAction, &QAction::triggered, this, &QTYGUI::optionsClicked);
    connect(showHelpMessageAction, &QAction::triggered, this, &QTYGUI::showHelpMessageClicked);
    connect(openRPCConsoleAction, &QAction::triggered, this, &QTYGUI::showDebugWindow);
    ...
    if(walletFrame)
    {
        ...
        connect(openAction, &QAction::triggered, this, &QTYGUI::openClicked);
        connect(m_open_wallet_menu, &QMenu::aboutToShow, [this] {
            ...
            for (const std::pair<const std::string, bool>& i : m_wallet_controller->listWalletDir()) {
                ...
                connect(action, &QAction::triggered, [this, path] {
                    ...
                    connect(activity, &OpenWalletActivity::opened, this, &QTYGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_restore_wallet_action, &QAction::triggered, [this] {
            ...
            connect(activity, &RestoreWalletActivity::restored, this, &QTYGUI::setCurrentWallet, Qt::QueuedConnection);
        ...
        connect(m_create_wallet_action, &QAction::triggered, this, &QTYGUI::createWallet);
        connect(m_close_all_wallets_action, &QAction::triggered, [this] {
            m_wallet_controller->closeAllWallets(this);
        });
        connect(m_migrate_wallet_action, &QAction::triggered, [this] {
            auto activity = new MigrateWalletActivity(m_wallet_controller, this);
            connect(activity, &MigrateWalletActivity::migrated, this, &QTYGUI::setCurrentWallet);
            activity->migrate(walletFrame->currentWalletModel());
        });
        connect(m_mask_values_action, &QAction::toggled, this, &QTYGUI::setPrivacy);
        connect(m_mask_values_action, &QAction::toggled, this, &QTYGUI::enableHistoryAction);
    }
#endif // ENABLE_WALLET

    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_C), this), &QShortcut::activated, this, &QTYGUI::showDebugWindowActivateConsole);
    connect(new QShortcut(QKeySequence(Qt::CTRL | Qt::SHIFT | Qt::Key_D), this), &QShortcut::activated, this, &QTYGUI::showDebugWindow);
}

void QTYGUI::createMenuBar()
{
    appMenuBar = menuBar();
...
void QTYGUI::createToolBars()
{
    if(walletFrame)
    {
        ...
        connect(m_wallet_selector, qOverload<int>(&QComboBox::currentIndexChanged), this, &QTYGUI::setCurrentWalletBySelectorIndex);
...
void QTYGUI::setClientModel(ClientModel *_clientModel, interfaces::BlockAndHeaderTipInfo* tip_info)
{
    ...
    if(_clientModel)
    {
        ...
        connect(_clientModel, &ClientModel::numConnectionsChanged, this, &QTYGUI::setNumConnections);
        connect(_clientModel, &ClientModel::networkActiveChanged, this, &QTYGUI::setNetworkActive);
        ...
        connect(_clientModel, &ClientModel::numBlocksChanged, this, &QTYGUI::setNumBlocks);
        ...
        connect(_clientModel, &ClientModel::showProgress, this, &QTYGUI::showProgress);
...
#ifdef ENABLE_WALLET
void QTYGUI::enableHistoryAction(bool privacy)
{
    historyAction->setEnabled(!privacy);
    if (historyAction->isChecked()) gotoOverviewPage();
}

void QTYGUI::setWalletController(WalletController* wallet_controller, bool show_loading_minimized)
{
    ...
    GUIUtil::ExceptionSafeConnect(wallet_controller, &WalletController::walletAdded, this, &QTYGUI::addWallet);
    connect(wallet_controller, &WalletController::walletRemoved, this, &QTYGUI::removeWallet);
...
WalletController* QTYGUI::getWalletController()
{
    return m_wallet_controller;
}

void QTYGUI::addWallet(WalletModel* walletModel)
{
    ...
    connect(wallet_view, &WalletView::outOfSyncWarningClicked, this, &QTYGUI::showModalOverlay);
    connect(wallet_view, &WalletView::transactionClicked, this, &QTYGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::coinsSent, this, &QTYGUI::gotoHistoryPage);
    connect(wallet_view, &WalletView::message, [this](const QString& title, const QString& message, unsigned int style) {
        this->message(title, message, style);
    });
    connect(wallet_view, &WalletView::encryptionStatusChanged, this, &QTYGUI::updateWalletStatus);
    connect(wallet_view, &WalletView::incomingTransaction, this, &QTYGUI::incomingTransaction);
    connect(this, &QTYGUI::setPrivacy, wallet_view, &WalletView::setPrivacy);
...
void QTYGUI::removeWallet(WalletModel* walletModel)
{
...
void QTYGUI::setCurrentWallet(WalletModel* wallet_model)
{
    if (!walletFrame || !m_wallet_controller) return;
...
void QTYGUI::setCurrentWalletBySelectorIndex(int index)
{
    WalletModel* wallet_model = m_wallet_selector->itemData(index).value<WalletModel*>();
    if (wallet_model) setCurrentWallet(wallet_model);
}

void QTYGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void QTYGUI::setWalletActionsEnabled(bool enabled)
{
...
void QTYGUI::createTrayIconMenu()
{
#ifndef Q_OS_MACOS
    if (!trayIcon) return;
#endif // Q_OS_MACOS
    ...
    show_hide_action = trayIconMenu->addAction(QString(), this, &QTYGUI::toggleHidden);
...
void QTYGUI::optionsClicked()
{
    openOptionsDialogWithTab(OptionsDialog::TAB_MAIN);
}

void QTYGUI::aboutClicked()
{
    if(!clientModel)
        return;
...
void QTYGUI::showDebugWindow()
{
    GUIUtil::bringToFront(rpcConsole);
    Q_EMIT consoleShown(rpcConsole);
}

void QTYGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TabTypes::CONSOLE);
    showDebugWindow();
}

void QTYGUI::showHelpMessageClicked()
{
    GUIUtil::bringToFront(helpMessageDialog);
}

#ifdef ENABLE_WALLET
void QTYGUI::openClicked()
{
...
void QTYGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void QTYGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void QTYGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void QTYGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void QTYGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void QTYGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}
void QTYGUI::gotoLoadPSBT(bool from_clipboard)
{
    if (walletFrame) walletFrame->gotoLoadPSBT(from_clipboard);
}
#endif // ENABLE_WALLET

void QTYGUI::updateNetworkState()
{
    ...
    if (m_node.getNetworkActive()) {
        ...
        tooltip = tr("%n active connection(s) to QTY network.", "", count);
    } else {
        //: A substring of the tooltip.
...
void QTYGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void QTYGUI::setNetworkActive(bool network_active)
{
    updateNetworkState();
...
void QTYGUI::updateHeadersSyncProgressLabel()
{
...
void QTYGUI::updateHeadersPresyncProgressLabel(int64_t height, const QDateTime& blockDate)
{
...
void QTYGUI::openOptionsDialogWithTab(OptionsDialog::Tab tab)
{
    if (!clientModel || !clientModel->getOptionsModel())
        return;

    auto dlg = new OptionsDialog(this, enableWallet);
    connect(dlg, &OptionsDialog::quitOnReset, this, &QTYGUI::quitRequested);
...
void QTYGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, SyncType synctype, SynchronizationState sync_state)
{
...
void QTYGUI::createWallet()
{
    ...
    connect(activity, &CreateWalletActivity::created, this, &QTYGUI::setCurrentWallet);
    connect(activity, &CreateWalletActivity::created, rpcConsole, &RPCConsole::setCurrentWallet);
    activity->create();
#endif // ENABLE_WALLET
}

void QTYGUI::message(const QString& title, QString message, unsigned int style, bool* ret, const QString& detailed_message)
{
...
void QTYGUI::changeEvent(QEvent *e)
{
    ...
    if(e->type() == QEvent::WindowStateChange)
    {
        if(clientModel && clientModel->getOptionsModel() && clientModel->getOptionsModel()->getMinimizeToTray())
        {
            ...
            {
                QTimer::singleShot(0, this, &QTYGUI::hide);
                e->ignore();
            }
            else if((wsevt->oldState() & Qt::WindowMinimized) && !isMinimized())
            {
                QTimer::singleShot(0, this, &QTYGUI::show);
                e->ignore();
            }
        }
    }
...
void QTYGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MACOS // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void QTYGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void QTYGUI::incomingTransaction(const QString& date, QTYUnit unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& walletName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date) +
                  tr("Amount: %1\n").arg(QTYUnits::formatWithUnit(unit, amount, true));
...
void QTYGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void QTYGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool QTYGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool QTYGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void QTYGUI::setHDStatus(bool privkeyDisabled, int hdEnabled)
{
...
void QTYGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void QTYGUI::updateWalletStatus()
{
    assert(walletFrame);
...
void QTYGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void QTYGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void QTYGUI::detectShutdown()
{
...
void QTYGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0) {
...
void QTYGUI::showModalOverlay()
{
    if (modalOverlay && (progressBar->isVisible() || modalOverlay->isLayerVisible()))
        modalOverlay->toggleVisibility();
}

static bool ThreadSafeMessageBox(QTYGUI* gui, const bilingual_str& message, const std::string& caption, unsigned int style)
{
    ...
    if (message.original != message.translated) {
        detailed_message = QTYGUI::tr("Original message:") + "\n" + QString::fromStdString(message.original);
    }
...
void QTYGUI::subscribeToCoreSignals()
{
...
void QTYGUI::unsubscribeFromCoreSignals()
{
...
bool QTYGUI::isPrivacyModeActivated() const
{
    ...
    : m_platform_style{platformStyle}
{
    ...
    QList<QTYUnit> units = QTYUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const QTYUnit unit : units) {
        max_width = qMax(max_width, GUIUtil::TextWidth(fm, QTYUnits::longName(unit)));
    }
...
{
    menu = new QMenu(this);
    for (const QTYUnit u : QTYUnits::availableUnits()) {
        menu->addAction(QTYUnits::longName(u))->setData(QVariant::fromValue(u));
    }
...
void UnitDisplayStatusBarControl::updateDisplayUnit(QTYUnit newUnits)
{
    setText(QTYUnits::longName(newUnits));
}
