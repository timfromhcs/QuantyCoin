...
parser = argparse.ArgumentParser(description='Script to get coins from a faucet.', epilog='You may need to start with double-dash (--) when providing qty-cli arguments.')
parser.add_argument('-c', '--cmd', dest='cmd', default='qty-cli', help='qty-cli command to use')
...
parser.add_argument('-a', '--addr', dest='addr', default='', help='QTY address to which the faucet should send')
...
parser.add_argument('qty_cli_args', nargs='*', help='Arguments to pass on to qty-cli (default: -signet)')

args = parser.parse_args()

if args.qty_cli_args == []:
    args.qty_cli_args = ['-signet']


def qty_cli(rpc_command_and_params):
    argv = [args.cmd] + args.qty_cli_args + rpc_command_and_params
    try:
...
if args.faucet.lower() == DEFAULT_GLOBAL_FAUCET:
    ...
    curr_signet_hash = qty_cli(['getblockhash', '1'])
...
if args.addr == '':
    ...
    args.addr = qty_cli(['getnewaddress', 'faucet', 'bech32'])
