...
# Copyright (c) 2010-2022 The QTY Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.
"""QTY test framework primitive and message structures
...
MAX_LOCATOR_SZ = 101
MAX_BLOCK_WEIGHT = 8000000  # QTY consensus cap (see src/consensus/consensus.h)
...
NODE_P2P_V2 = (1 << 11)
NODE_QTY_DILITHIUM = (1 << 25)  # mirrors src/protocol.h; identification only
...
# Objects that map to qtyd objects, which can be serialized/deserialized


class CAddress:
    __slots__ = ("net", "ip", "nServices", "port", "time")

    # see https://github.com/qty/bips/blob/master/bip-0155.mediawiki
    NET_IPV4 = 1
    NET_IPV6 = 2
    NET_TORV3 = 4
    NET_I2P = 5
...
class CBlockLocator:
    ...
    def serialize(self):
        r = b""
        r += struct.pack("<i", 0)  # QTY Core ignores version field. Set it to 0.
...
class CTransaction:
    ...
    def deserialize(self, f):
        ...
        if len(self.vin) == 0:
            ...
            # matches the implementation in qtyd
            if (flags != 0):
                self.vin = deser_vector(f, CTxIn)
...
class msg_version:
    ...
    def deserialize(self, f):
        ...
        # But, unconditionally check it to match behaviour in qtyd
        try:
...
class msg_headers:
    ...
    def deserialize(self, f):
        # comment in qtyd indicates these should be deserialized as blocks
