...
"""Encode and decode QTY addresses.
...
# QTY: Use qcqty1 prefix for QTY regtest addresses
ADDRESS_BCRT1_UNSPENDABLE = 'qcqty1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqen882c'
ADDRESS_BCRT1_UNSPENDABLE_DESCRIPTOR = 'addr(qcqty1qqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqqen882c)#uc8x94y3'
# Coins sent to this address can be spent with a witness stack of just OP_TRUE
ADDRESS_BCRT1_P2WSH_OP_TRUE = 'qcqty1qft5p2uhsdcdc3l2ua4ap5qqfg4pjaqlp250x7us7a8qqhrxrxfsqcvxxf7'
...
def keyhash_to_p2pkh(hash, main=False):
    assert len(hash) == 20
    # QTY: Use QTY-specific base58 prefixes
    version = 75 if main else 111  # QTY mainnet uses 75, testnet/regtest uses 111
    return byte_to_base58(hash, version)

def scripthash_to_p2sh(hash, main=False):
    assert len(hash) == 20
    # QTY: Use QTY-specific base58 prefixes
    version = 135 if main else 196  # QTY mainnet uses 135, testnet/regtest uses 196
    return byte_to_base58(hash, version)
...
def program_to_witness(version, program, main=False):
    ...
    # QTY: Use QTY-specific HRP for regtest
    return encode_segwit_address("qty" if main else "qcqty", version, program)
...
def bech32_to_bytes(address):
    ...
    # Accept both Bitcoin HRPs and QTY HRPs to keep tests flexible
    if hrp not in ['bc', 'tb', 'bcrt', 'qty', 'qty', 'qcqty']:
        return (None, None)
