...
""" Example logging QTY Core mempool events using the mempool:added,
...
def main(qtyd_path):
    qtyd_with_usdts = USDT(path=str(qtyd_path))

    # attaching the trace functions defined in the BPF program
    # to the tracepoints
    qtyd_with_usdts.enable_probe(probe="mempool:added", fn_name="trace_added")
    qtyd_with_usdts.enable_probe(probe="mempool:removed", fn_name="trace_removed")
    qtyd_with_usdts.enable_probe(probe="mempool:replaced", fn_name="trace_replaced")
    qtyd_with_usdts.enable_probe(probe="mempool:rejected", fn_name="trace_rejected")
    bpf = BPF(text=PROGRAM, usdt_contexts=[qtyd_with_usdts])
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("USAGE: ", sys.argv[0], "path/to/qtyd")
        exit(1)
