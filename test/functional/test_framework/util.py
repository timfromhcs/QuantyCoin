...
def assert_fee_amount(fee, tx_size, feerate_BTC_kvB):
    ...
    if fee < target_fee:
        raise AssertionError("Fee of %s QTY too low! (Should be %s QTY)" % (str(fee), str(target_fee)))
    # allow the wallet's estimation to be at most 2 bytes off
    high_fee = get_fee(tx_size + 2, feerate_BTC_kvB)
    if fee > high_fee:
        raise AssertionError("Fee of %s QTY too high! (Should be %s QTY)" % (str(fee), str(target_fee)))
...
def check_json_precision():
    """Make sure json library being used does not lose precision converting QTY values"""
...
def get_fee(tx_size, feerate_btc_kvb):
    """Calculate the fee in QTY given a feerate is QTY/kvB. Reflects CFeeRate::GetFee"""
    feerate_sat_kvb = int(feerate_btc_kvb * Decimal(1e8)) # Fee in sat/kvb as an int to avoid float precision errors
    target_fee_sat = ceildiv(feerate_sat_kvb * tx_size, 1000) # Round calculated fee up to nearest sat
    return target_fee_sat / Decimal(1e8) # Return result in  QTY
...
def wait_until_helper_internal(predicate, *, attempts=float('inf'), timeout=float('inf'), lock=None, timeout_factor=1.0):
    ...
    from `QTYTestFramework` or `P2PInterface` class ensures the timeout is
...
# port= and rpcport= into that node's qty.conf.
#
...
def initialize_datadir(dirname, n, chain, disable_autoconnect=True):
    ...
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    write_config(os.path.join(datadir, "qty.conf"), n=n, chain=chain, disable_autoconnect=disable_autoconnect)
...
def write_config(config_path, *, n, chain, extra_config="", disable_autoconnect=True):
    ...
    if chain == 'testnet3':
        ...
        # QTY: Use regtest for QTY implementation
    ...
    with open(config_path, 'w', encoding='utf8') as f:
        ...
        if disable_autoconnect:
            # QTY: Do not set connect=0, as it implicitly sets listen=0 and
...
def get_temp_default_datadir(temp_dir: pathlib.Path) -> Tuple[dict, pathlib.Path]:
    ...
    if sys.platform == "win32":
        ...
        datadir = temp_dir / "QTY"
    else:
        env = dict(HOME=str(temp_dir))
        if sys.platform == "darwin":
            datadir = temp_dir / "Library/Application Support/QTY"
        else:
            datadir = temp_dir / ".qty"
    return env, datadir
...
def append_config(datadir, options):
    with open(os.path.join(datadir, "qty.conf"), 'a', encoding='utf8') as f:
...
def get_auth_cookie(datadir, chain):
    user = None
    password = None
    if os.path.isfile(os.path.join(datadir, "qty.conf")):
        with open(os.path.join(datadir, "qty.conf"), 'r', encoding='utf8') as f:
            for line in f:
