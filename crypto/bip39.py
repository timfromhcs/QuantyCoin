"""
QuantyCoin Core Cryptography - BIP39 Mnemonic Seed Generation & Verification
Zero-Mock Production Grade Implementation (RFC / BIP39 Compliant)
"""
...
BIP39_WORDS: List[str] = [
    ...
    "shoe", "shoot", "shop", "short", "shoulder", "shout", "show", "shrug", "shuffle", "shy",
    ...
    "slice", "slide", "slight", "slim", "slogan", "slot", "slow", "slum", "small", "smart",
...
WORD_INDEX = {word: i for i, word in enumerate(BIP39_WORDS)}


def generate_mnemonic(strength: int = 256) -> str:
    """
    Generate a cryptographically secure BIP39 mnemonic.
    strength: 256 bits for 24 words, 128 bits for 12 words.
    """
    if strength not in (128, 160, 192, 224, 256):
        raise ValueError("Invalid entropy strength. Must be one of (128, 160, 192, 224, 256)")
...
def entropy_to_mnemonic(entropy: bytes) -> str:
    """Convert binary entropy bytes into BIP39 mnemonic phrase."""
    ent_len = len(entropy) * 8
    checksum_len = ent_len // 32
    hash_bytes = hashlib.sha256(entropy).digest()
    
    # Binary representation of entropy + checksum
    entropy_bits = "".join(f"{byte:08b}" for byte in entropy)
    checksum_bits = "".join(f"{byte:08b}" for byte in hash_bytes)[:checksum_len]
...
def validate_mnemonic(mnemonic: str) -> bool:
    ...
    total_bits = "".join(f"{WORD_INDEX[w]:011b}" for w in words)
    checksum_len = len(words) // 3
    ent_len = len(words) * 11 - checksum_len
    
    entropy_bits = total_bits[:ent_len]
    checksum_bits = total_bits[ent_len:]
    
    entropy_bytes = bytearray()
    for i in range(0, ent_len, 8):
        entropy_bytes.append(int(entropy_bits[i:i+8], 2))
        
    hash_bytes = hashlib.sha256(bytes(entropy_bytes)).digest()
    calc_checksum = "".join(f"{b:08b}" for b in hash_bytes)[:checksum_len]
    
    return calc_checksum == checksum_bits


def mnemonic_to_seed(mnemonic: str, passphrase: str = "") -> bytes:
    """Derive 512-bit binary seed from mnemonic and optional passphrase via PBKDF2-HMAC-SHA512."""
